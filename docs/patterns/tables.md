...
| Can reorder items in the table!                  | `@cp949/react-beautiful-dnd` 😎 |

`@cp949/react-beautiful-dnd` requires no additional wrapping elements to create `<Draggable />` and `<Droppable />` components. Therefore it is possible to have a `<table>` that has valid HTML as well as supporting drag and drop.
