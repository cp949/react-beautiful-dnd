...
This page goes over the design and interaction thinking behind `@cp949/react-beautiful-dnd`.

## Foundational idea: physicality

The core design idea of `@cp949/react-beautiful-dnd` is physicality: we want users to feel like they are moving physical objects around
...
`@cp949/react-beautiful-dnd` relies on movement to communicate positioning. It is trying to create a system that is based on physical metaphores. Drop shadows, lines and other affordances are useful in drag and drop contexts where natural movement is not possible.
